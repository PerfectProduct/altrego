...
Сценария у чекера нет: S-02 держит `claim_provenance` и перечисляет свои три
формы закрытым списком, а этот класс в нём не описан. Это названная цена, а не
молчаливый пробел — черновик сценария уходит владельцу пакетом P3 (ярус 2).
