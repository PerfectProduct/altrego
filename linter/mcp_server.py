...
1. Все пути абсолютные — от `__file__`, из аргументов запуска либо из
   окружения; на cwd не опирается ничто.
2. Путь вольта приходит АРГУМЕНТОМ ЗАПУСКА (`--vault-master`), а окружение
   остаётся вторым каналом. Блок `env` записи сервера до процесса внутри
   дистрибутива не доходит: измерено 2026-09-08 в PowerShell, оба направления
   с кодами выхода — `wsl.exe -e printenv ALTREGO_VAULT_MASTER` печатал пусто
   и выходил кодом 1, а путь и код 0 появились только под
   `WSLENV=ALTREGO_VAULT_MASTER/u`. Не задано ни аргументом, ни переменной —
   читаемый отказ инструмента (`vault_env_unset`), называющий ОБА канала, а не
   молчаливое умолчание на каталог владельца (CLAUDE.md, «Мост MCP»).
...
def _reexec_or_refuse(missing: list[str]) -> None:
    ...
    Аргументы запуска передаются дальше дословно (`*sys.argv[1:]`): путь вольта
    приходит именно ими (см. «аргументы запуска» ниже), и потерянный на гарде
    аргумент дал бы `vault_env_unset` при верно заданном пути — отказ на месте
    рабочей связки. Гард стоит ДО разбора аргументов намеренно: отказ опоры
    первичнее разбора, а неизвестный флаг чужой интерпретатор донесёт до
    перезапущенного процесса, где argparse его и назовёт.
...
# ──────────────────── аргументы запуска: пути вольта ─────────────────────────
#
# Путь вольта приходит аргументом командной строки, а не только окружением.
# Повод измерен 2026-09-08 (PowerShell, оба направления, коды выхода записаны):
# блок `env` записи сервера достаётся ПРОЦЕССУ WINDOWS `wsl.exe` и внутрь
# дистрибутива не переносится. `wsl.exe -e printenv ALTREGO_VAULT_MASTER`
# печатал пусто и выходил кодом 1; путь и код 0 появились только после
# `WSLENV=ALTREGO_VAULT_MASTER/u`. Командная строка же доходит целиком — тем же
# каналом, которым до сервера доходит путь к нему самому.
#
# Поддержки `WSLENV` в коде нет намеренно: это данная ТРАНСПОРТА клиента, и
# мост, читающий её, зависел бы от механики конкретной связки вместо своего
# предмета. Аргумент от механики транспорта не зависит вовсе.
#
# Окружение не отменяется, а становится вторым каналом: переменной одной
# по-прежнему достаточно там, где она до процесса доходит, — запуск из WSL
# руками, харнесс, CI. (Прежняя запись клиента этим не спасается: она пути и не
# доносила.) Умолчания на каталог по-прежнему нет ни одного.
#
# `--vault-clone` сегодня не читает ни один инструмент (клон нужен пересчёту
# хэшей, а он идёт прогоном `--full` владельца). Флаг заведён парой к
# переменной, чтобы обе двери адресовались одинаково и запись клиента была
# полной; разрешается он тем же порядком, что и master.

VAULT_OPTIONS = {run.ENV_MASTER: "--vault-master", run.ENV_CLONE: "--vault-clone"}


class ArgParser(argparse.ArgumentParser):
    """Разбор аргументов, которому stdout закрыт.

    argparse печатает справку в stdout, а stdout здесь занят кадрами JSON-RPC:
    строка справки там не сообщение, а сломанный кадр протокола. Обе двери
    вывода уводятся в stderr — единственный канал, по которому до клиента
    доходит что-либо, кроме протокола.
    """
...
def parse_args(argv: list[str]) -> dict[str, str]:
    """Пути вольта из командной строки: {имя переменной: путь}.

    Ключ — имя переменной окружения, а не имя флага: дальше «чем задан путь»
    спрашивается в одном месте (`vault_path`), и два канала не расползаются по
    коду.

    Пустое значение считается незаданным — ровно как пустая переменная в
    `run.env_path`: иначе `--vault-master ""` дал бы путь `""`, то есть тихий
    относительный путь от cwd клиента (`/mnt/c/Windows/system32` — измерено).

    Неизвестный аргумент — отказ argparse кодом 2 текстом в stderr: читаемый
    отказ, а не трассировка и не строка в stdout.
    """
    parser = ArgParser(
        prog=SERVER_NAME,
        description="stdio-MCP-мост ход-линтера полосы altrego")
    for name, option in VAULT_OPTIONS.items():
        parser.add_argument(option, dest=name, metavar="ПУТЬ", default=None,
                            help=f"путь вольта; второй канал — переменная {name}")
    given = vars(parser.parse_args(argv))
    return {name: value.strip() for name, value in given.items()
            if value and value.strip()}


# Разбор идёт до построения сервера — и только когда файл запущен точкой входа:
# при импорте (харнесс, чужой модуль) `sys.argv` принадлежит другой программе, и
# argparse отказал бы на её флагах.
CLI_VAULT: dict[str, str] = parse_args(sys.argv[1:]) if __name__ == "__main__" else {}


def vault_path(name: str) -> str | None:
    """Путь вольта по имени переменной: аргумент запуска, затем окружение.

    Порядок именно такой: аргумент — канал, который транспорт клиента доносит
    целиком, окружение — тот, который доносит не всегда. Ни того ни другого —
    `None`, и вызвавший обязан отказать: умолчания на каталог владельца в
    репозитории нет (CLAUDE.md, «Пути вольта — переменные окружения»).
    """
    return CLI_VAULT.get(name) or run.env_path(name)
...
def queue() -> QueueHead:
    ...
    Путь вольта приходит аргументом запуска (`--vault-master`), окружение —
    второй канал. Блок `env` записи клиента до процесса внутри дистрибутива не
    доходит (измерено 2026-09-08), поэтому путь, не заданный ни одним из двух
    каналов, — названный отказ с лечением, а не умолчание на каталог владельца.
    """
    config = _config()
    master = vault_path(run.ENV_MASTER)
    if not master:
        option = VAULT_OPTIONS[run.ENV_MASTER]
        raise ToolError(
            f"vault_env_unset: рабочая копия вольта не адресована — ни "
            f"аргументом запуска `{option} ПУТЬ`, ни переменной окружения "
            f"{run.ENV_MASTER}; умолчания на каталог владельца в репозитории "
            f"нет. Основной канал — аргумент: клиент поднимает сервер через "
            f"`wsl.exe`, и блок `env` его записи достаётся процессу Windows, а "
            f"внутрь дистрибутива не попадает (измерено 2026-09-08). Лечение: "
            f"дописать `{option} <путь>` в `args` записи сервера и перезапустить "
            f"клиента; переменная окружения работает вторым каналом там, где "
            f"она до процесса доходит")
